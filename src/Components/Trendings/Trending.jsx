...
const Trending = ({ flower }) => {
  ...
  return (
    <div className="bg-base-200 rounded shadow-md overflow-hidden w-full max-w-sm flex flex-col hover:shadow-lg transition duration-300">
      {/* ⬇️ Image height made smaller */}
      <img src={img} alt={title} className="w-full h-52 p-5 object-cover rounded" />

      <div className="flex flex-col justify-between flex-grow px-4 pb-4 space-y-3">
        ...
        <Link to={`/singleCard/${flower.id}`}>
          <button className=" btn bg-red-500  hover:bg-pink-600 text-white py-2 px-6 rounded font-medium transition">
            Veiw Details
          </button>
        </Link>
      </div>
